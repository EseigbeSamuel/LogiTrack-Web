...
- **Accents**: High-Vis fluorescent tones (Neon Green) for crucial metrics, active trackers, and primary actions. Refined yellow for secondary highlights.
...
- **Component Library**: The project strictly utilizes **Shadcn UI** for all interactive elements (Buttons, Inputs, Selects, Tables, etc.) to ensure global consistency.
- **Nomenclature**: The system uses the term **"Fleet"** (not "Vehicle") for all transportation units.

---

## 2. Typography
- **Primary Font**: Clean, modern sans-serif (e.g., `Inter`, `Outfit`, or system default `sans-serif`).
- **Tactical/Plate Font**: Monospace (`SFMono-Regular`, `Consolas`, or `Courier New`) for fleet registration plates, coordinates, and timestamp values.
...
| **Primary (Highlight)** | `#CCFF00` | `oklch(0.93 0.25 115)` | Active state indicator, Primary CTAs, active route paths. All prominent buttons use this color. |
| **Accent (Highlight)** | `#FDE047` | `oklch(0.9 0.15 95)` | Refined bright yellow for warnings, delayed statuses, and secondary alerts (replacing the previous muddy/brownish yellow). |
...
1. **Primary Button**:
   ...
   - *Note*: Buttons previously using the accent color have been migrated to the Primary color.
...
### 5.2 Status Badges
Used to represent fleet availability states:
...
*   **Maintenance**:
    *   Text: `#FDE047` (Bright Refined Yellow)
    *   Background: Bright Yellow with 10% opacity (`#FDE0471A`)

### 5.3 Input Fields (Shadcn UI)
